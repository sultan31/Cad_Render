                            ...
                            <div class="row mt-2">
                                <div class="col-12">
                                    <table class="table table-hover table-bordered m-0 table-centered nowrap w-100" id="tickets-table">
                                        ...
                                        $jobs_data =  $this->db->query("SELECT id, SUM(`diff_time`) as t_diff_time, SUM(`actual_time`) as t_actual_time, job_id, status, diff_name  FROM `cad_action_time` WHERE `user_id` = ".$id." AND `created_date` >= '".$from_date." 00:00:00' AND `created_date` <= '".$to_date." 23:59:59' ".$dept_filter." GROUP BY `status`, `job_id` ORDER BY job_id ASC");
                                        ...
                                        ?>
                                                    <tr>
                                                        <td><?php echo $this->master_model->get_one_record('production_order', 'order_number', $value['job_id']); ?></td>
                                                        <td><?php
                                                     ...
                                                     <td><?php echo $value['diff_name'];?></td>
                                                        <td><?php 
                                                         ...
                                                         ?></td>
                                                        <td><?php echo $this->master_model->get_one_record('status', 'status_name', $value['status']);?></td>
...
<script type="text/javascript">
    ...
    function export_csv()
    {
       ...
       var to_date = '<?php echo !empty($this->uri->segment(5)) ? $this->uri->segment(5) : '' ;?>';
        ...
        window.open(
                "<?php echo base_url()?>cad_production_report/export_csv_details?user_id="+user_id+"&from_date="+from_date+"&to_date="+to_date,
    ...
    function view_detail()
    {
       ...
       var to_date = '<?php echo !empty($this->uri->segment(5)) ? $this->uri->segment(5) : '' ;?>';
        ...
        window.open(
                "<?php echo base_url()?>reports/view_detail?user_id="+user_id+"&from_date="+from_date+"&to_date="+to_date,
                "_self" // <- This is what makes it open in a new window.
            );
            return false;
    }
