                ...
                <div class="row">
                    ...
                    <div class="col-12">
                        ...
                        <div class="card-box">
                            <form action="" method="get">
                                    ...
                                    <div class="form-group col-4">
                                        ...
                                        <select class="form-control" name="user_id" id="user_id" data-toggle="select2">
                                            ...
                                            if($users->num_rows() > 0)
                                            {
                                                ...
                                                foreach($users as $c)
                                                {
                                                    $selected = isset($_REQUEST['user_id']) && $_REQUEST['user_id'] != '' && $_REQUEST['user_id'] == $c['id'] ? 'selected' : '';
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-4">
                                        ...
                                        <select class="form-control select2-multiple" data-toggle="select2" multiple="multiple" data-placeholder="Choose ..." name="department_id[]" id="department_id">
                                    ...
                                    <div class="form-group col-4">
                                        ...
                                        <select class="form-control" name="status_name" id="status_name" data-toggle="select2">
                                            ...
                                            if($status->num_rows() > 0)
                                            {
                                                ...
                                                foreach($status as $s)
                                                {
                                                    $selected1 = isset($_REQUEST['status_name']) && $_REQUEST['status_name'] != '' && $_REQUEST['status_name'] == $s['label'] ? 'selected' : '';
                                                    echo '<option value="'.$s['label'].'" '.$selected1.'>'.$s['label'].'</option>';
                                                }
                                            }
                                            ?>
                                        </select>
                            ...
                            <div class="row mt-2">
                                <div class="col-12">
                                    <table class="table table-hover table-bordered m-0 table-centered w-100" id="tickets-table">
                                        ...
                                        if(!empty($_REQUEST))
                                        {
                                            ...
                                            if(isset($_REQUEST['status_name']) && $_REQUEST['status_name'] != '')
                                            {
                                                ...
                                                {
                                                    // $department_id = implode(',', $_REQUEST['department_id']);
                                                    $this->db->where('`a`.`status` IN (SELECT `id` FROM `ms_render_status` WHERE label = "'.$_REQUEST['status_name'].'" AND department_id IN ('.$department_child_id.'))', NULL, FALSE);                                        
                                                }
                                                else
                                                {
                                                    $this->db->where('`a`.`status` IN (SELECT `id` FROM `ms_render_status` WHERE label = "'.$_REQUEST['status_name'].'")', NULL, FALSE);
                                                }
                                            ...
                                            {
                                                $this->db->where('a.assigned_to', $_REQUEST['user_id']);
                                            }
                                            ...
                                            if(isset($_REQUEST['b.po_no']) && $_REQUEST['po_no'] != '')
                                            {
                                            ...
                                            foreach($row as $r)
                                            {
                                                ...
                                                <tr id="order_id_"<?php echo $r['id'];?> class="<?php echo $highlight_tr;?>">
                                                    ...
                                                    <td><?php
                                                             $dept_name = $this->db->query('SELECT dept_name FROM department WHERE id = '.$r['department_id'])->result_array();
                                                             $dept_name = array_key_exists('0', $dept_name) ? $dept_name[0]['dept_name'] : '';
                                                             echo $dept_name;?></td>
                                                    ...
                                                    <td><?php 
                                                        $difficulty_query = $this->db->query('SELECT difficulty_id FROM `render_order_log` WHERE `job_id` = '.$r['id'].' AND difficulty_id != 0');
                                                        // pre($this->db->last_query());
...
<script type="text/javascript">
    $('#clear_filter').click(function(){
        ...
        window.location.href = '<?php echo base_url();?>render_reports/render_status_report';
    });
    ...
    function export_csv()
    {
       ...
       var department_id = '<?php echo isset($_REQUEST['department_id']) && !empty($_REQUEST['department_id']) ? implode(',', $_REQUEST['department_id']) : "";?>';

        window.open(
                "<?php echo base_url()?>render_reports/export_status_report_csv?order_no="+order_no+"&user_id="+user_id+"&status_name="+status_name+"&from_date="+from_date+"&to_date="+to_date+"&difficulty_name="+difficulty_name+"&client="+client+"&department_id="+department_id+"&po_no="+po_no,
