                                    ...
                                    <table class="table table-hover table-bordered m-0 table-centered w-100" id="tickets-table">
                                        ...
                                        if(!empty($_REQUEST))
                                        {
                                            ...
                                            if(isset($_REQUEST['status_name']) && $_REQUEST['status_name'] != '')
                                            {
                                                ...
                                                {
                                                    // $department_id = implode(',', $_REQUEST['department_id']);
                                                    $this->db->where('`status` IN (SELECT `id` FROM `ms_render_status` WHERE label = "'.$_REQUEST['status_name'].'" AND department_id IN ('.$department_child_id.'))', NULL, FALSE);                                        
                                                }
                                                else
                                                {
                                            ...
                                            {
                                                $this->db->like('order_number', trim($_REQUEST['order_no']));
                                            }

                                            if(isset($_REQUEST['po_no']) && $_REQUEST['po_no'] != '')
                                            {
                                                $this->db->like('po_no', trim($_REQUEST['po_no']));
                                            }
                                            ...
                                            {
                                                // $department_id = implode(',', $_REQUEST['department_id']);
                                                $this->db->where('`department_id` IN ('.$department_child_id.')', NULL, FALSE);
                                           ...
                                           # pre($this->db->last_query());
                                            ...
                                            foreach($row as $r)
                                            {
                                                ...
                                                <tr id="order_id_"<?php echo $r['id'];?> class="<?php echo $highlight_tr;?>">
                                                    ...
                                                    <td><?php echo $this->db->query('SELECT dept_name FROM department WHERE id = '.$r['department_id'])->result_array()[0]['dept_name'];;?></td>
...
<script type="text/javascript">
    $('#clear_filter').click(function(){
        ...
        window.location.href = '<?php echo base_url();?>render_reports';
    });
    ...
    function export_csv()
    {
       ...
       var department_id = '<?php echo isset($_REQUEST['department_id']) && !empty($_REQUEST['department_id']) ? implode(',', $_REQUEST['department_id']) : "";?>';
        ...
        window.open(
                "<?php echo base_url()?>render_reports/export_csv?order_no="+order_no+"&user_id="+user_id+"&status_name="+status_name+"&from_date="+from_date+"&to_date="+to_date+"&difficulty_name="+difficulty_name+"&client="+client+"&department_id="+department_id+"&po_no="+po_no,
