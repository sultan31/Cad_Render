                                ...
                                <div class="col-12">
                                    <table class="table table-hover table-bordered m-0 table-centered nowrap w-100" id="tickets-table">
                                        ...
                                        <tr>
                                            ...
                                            <th>Total Time</th>
                                            <?php
                                                $status = $this->db->query('SELECT id,status_name FROM `status` WHERE show_in_report = 1  ORDER BY report_position ASC');
                                                if($status->num_rows() > 0)
                                                {
                                                    $status = $status->result_array();
                                                    foreach($status as $d)
                                                    {
                                                        echo '<th>'.$d['status_name'].'</th>';
                                                    }
                                                }
                                            ?>
                                            <th>PDW Sent</th>
                                        ...
                                        <tbody>
                                            ...
                                            if($users->num_rows() > 0)
                                            {
                                                ...
                                                foreach($users as $u)
                                                {
                                                    ...
                                                    <td><?php echo '<a href="'.base_url().'reports/detailed_report/'.$u['id'].'/'.$from_date.'/'.$to_date.' ">'.$u['full_name'].'</a>'; ?></td>
                                                    <td><?php  
                                                            $init = $get_total_time;
                                                            $hours = floor($init / 60);
                ...
                <form action="" method="get">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="filter_user_id" id="filter_user_id" data-toggle="select2">
                            ...
                            <?php
                            $users = $this->db->query('SELECT * FROM `users` WHERE user_role = 2');
                            if($users->num_rows() > 0)
                            {
                                $users = $users->result_array();
                                foreach($users as $c)
...
<script type="text/javascript">
    $('#clear_filter').click(function(){
        ...
        window.location.href = '<?php echo base_url();?>reports/cad_production';
    });
    ...
    function export_csv()
    {
       ...
       var to_date = '<?php echo isset($_REQUEST["to_date"]) ? $_REQUEST["to_date"] : "";?>'; 
        ...
        window.open(
                "<?php echo base_url()?>cad_production_report/export?user_id="+user_id+"&from_date="+from_date+"&to_date="+to_date,
