    ...
    public function fetch_records() {
       $today = date('Y-m-d');
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $column = array('order_number', 'po_no', 'category', 'client_name', 'order_date', 'file_path');/**  set your data base column name here for sorting* */
            ...
            if (isset($search['value']) && !empty($search['value'])) {
                ...
                {
                    $this->db->select('*');
                    $this->db->from('render_status_reports_view');
                    ...
                    {
                        $this->db->where('DATE_FORMAT(created_date, "%Y-%m-%d") >= ', $_REQUEST['from_date']);
                        
                    }
                    if(isset($_REQUEST['to_date']) && $_REQUEST['to_date'] != '')
                    {
                        $this->db->where('DATE_FORMAT(created_date, "%Y-%m-%d") <= ', $_REQUEST['to_date']);
                    }
            ...
            if (!empty($result->result_array())) {
                foreach ($result->result_array() as $k => $v) {
                        ...
                        {
                            $order_date = date('Y-m-d', strtotime($v['created_date']));
                        }
                        ...
                        $latest_act = isset($v['created_date']) && !empty($v['created_date']) ? date('d-m-y', strtotime($v['created_date'])).'<br>'.date('H:i', strtotime($v['created_date'])) : ''; 

                        $difficulty_query = $this->db->query('SELECT difficulty_id FROM `render_order_log` WHERE `job_id` = '.$v['id'].' AND difficulty_id != 0');
                        ...
                        $render_start_stop_time = $this->db->query('SELECT * FROM `render_start_stop_time` WHERE job_id = '.$v['id'].' ORDER BY id DESC LIMIT 1');
                        if($render_start_stop_time->num_rows() > 0)
                        {
                            $render_start_stop_time = $render_start_stop_time->result_array();
                            ...
                            $render_start_stop_action = $this->db->query('SELECT * FROM `render_start_stop_action` WHERE job_id = '.$v['id'].' ORDER BY id DESC LIMIT 1');
                            if($render_start_stop_action->num_rows() > 0)
                            {
                                $actual_time = $diff + $render_start_stop_action->result_array()[0]['actual_time'];
                    ...
                    $data[] = array(
                        ...
                        'status' =>  '<span class="badge badge-outline-success" style="color:#fff;background-color:'.$v['color'].';border:1px solid '.$v['color'].';">'.$v['label'].'</span>',
